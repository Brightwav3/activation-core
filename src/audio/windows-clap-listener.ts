...
export interface WindowsClapListenerOptions { sourceId: string; deviceId?: string; roomId?: string; device?: string; microphoneFactory?: () => Promise<MicrophoneStream>; }
...
export class WindowsClapListener {
  ...
  constructor(provider: DoubleClapProvider, private readonly options: WindowsClapListenerOptions) { this.bridge = new ClapAudioBridge(provider, { sourceId: options.sourceId, deviceId: options.deviceId, roomId: options.roomId, sampleRateHz: 16_000 }); }
  async start() {
